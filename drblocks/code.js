...
Code.renderContent = function() {
  ...
  if (content.id == 'content_xml') {
    ...
    Code.attemptCodeGeneration(Blockly.Ruby);
  }
  if (typeof PR == 'object') {
    PR.prettyPrint();
  }
...
Code.init = function() {
  ...
  toolboxText = toolboxText.replace(/(^|[^%]){(\w+)}/g,
      function(m, p1, p2) {return p1 + MSG[p2];console.log(p1, MSG[p2]);});
...
function text_to_file(text, filename) {
  var textfile = document.createElement('a');
  textfile.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(text));
  textfile.setAttribute('download', filename);
  textfile.style.display = 'none';
  document.body.appendChild(textfile);
  textfile.click();
  document.body.removeChild(textfile);
}

function export_xml() {
  var code = Blockly.Xml.domToPrettyText(Blockly.Xml.workspaceToDom(Code.workspace));
  text_to_file(code, "game.xml");
}

function export_ruby() {
  var code = Blockly.Ruby.workspaceToCode(Code.workspace);
  text_to_file(code, "main.rb");
}
...
function import_xml()  {
  ...
  fileSelector.addEventListener('change', function(e) {
    ...
    var file = e.target.files[fileList.length - 1];
